...

{
    ...
    while true ; do
        ...
        for i in "${tags[@]}" ; do
            case ${i:0:1} in
                '#')
                    ...
                    ;;
                '+')
                    echo -n "^bg(#888888)^fg(#991e99)"
                    ;;
                ':')
                    echo -n "^bg()^fg(#888888)"
                    ;;
                '!')
                    echo -n "^bg(#FF0675)^fg(#fff13e)"
                    ;;
                *)
                    #echo -n "^bg()^fg($selbg)"
                    continue
        ...
        # small adjustments
        baterry=$(<$baterry_path/capacity)%
        volume=$(awk -F"[][]" '/Left:/ { print $2 }' <(amixer sget Master))
        light=$(light)
        right="$separator^bg() V $volume $separator $date $separator L ${light/.00/%} $separator B $baterry"
